...
% repetition settings
cfg.tpb             = 60;        % presented frequencies in block
cfg.n_blocks        = 2;         % number of blocks
cfg.num_freqs       = 120;       % number of frequencies % can be lower
cfg.min_leap        = 0.65;      % minimum leap between stimulations in octives
cfg.temp_leap       = 3;         % steps to not use frequencies in similair range
cfg.per_gap         = 0.05;      % percentage silent gaps
cfg.n_gaps          = cfg.tpb*cfg.per_gap;   % number of gaps within a block


% timing settings
cfg.stim_len        = 0.2;      % stimulus presentation length in sec
cfg.isi             = 0.4;      % inter stimulus interval in sec > jitter
cfg.ibi             = 6;        % number of seconds silence between blocks
cfg.wait_onset      = 4;        % number of seconds silence before first trial (12.6 sec)
cfg.wait_offset     = 4;        % number of TRs silence after last trial
cfg.presdelay       = 0;        % time delay to present after pulse trigger (0ms here)

% timing/isi jitter
cfg.isimin          = 0.4;      % minimum isi
cfg.isimax          = 0.8;      % maximum isi
cfg.isisteps        = 6;        % number of jitters
cfg.isiz            = linspace(cfg.isimin, cfg.isimax, cfg.isisteps); % create isi's array
...
% generate jitters per trial
cfg.jitteridx   = randperm(length(cfg.stims));
cfg.jitters     = repelem(cfg.isiz, length(cfg.stims)/cfg.isisteps);
cfg.jitters     = cfg.jitters(cfg.jitteridx);
...
cfg.sound.nrchannels      = 2;                    % sellect number of channels (1=mono-audio)
...
%% SAVE SETTINGS
save (fullfile( pwd, 'data', ppnum, [ppnum '_settings_localizer.mat']));
